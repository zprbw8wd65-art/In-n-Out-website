...
def infer_body_style_from_url(url: str) -> str:
    """Body style, derived from which category page a vehicle was found
    on — far more reliable than trying to scrape a 'Body Style' label
    off the vehicle's own page (which was frequently missing/blank and
    is why the body-style filter was matching zero cars)."""
    mapping = [
        ("sedan-for-sale", "Sedan"),
        ("suvs-for-sale", "SUV"),
        ("pickup-trucks-for-sale", "Pickup"),
        ("wagon-for-sale", "Wagon"),
        ("full-size-for-sale", "Full Size"),
        ("minivans-for-sale", "Minivan"),
        ("chassis-for-sale", "Chassis"),
        ("hatchbacks-for-sale", "Hatchback"),
    ]
...
def parse_listing_page(html_text: str, page_url: str) -> list:
    ...
    vehicles = {}
    body_style = infer_body_style_from_url(page_url)
...
def find_label_value(soup, labels: list) -> str:
    """Generic key/value finder. Tries the structural approach first
    (label element, then its next sibling's text), then falls back to
    a line-based scan of the page's rendered text — this catches
    layouts where the label/value aren't simple adjacent siblings,
    which is why fields like Engine/Transmission were coming back
    empty even though Condition/Colors worked structurally."""
    for label in labels:
    ...
    # Fallback: scan the page's rendered text line-by-line. Carsforsale-
    # style detail pages typically show "Label" then "Value" as separate
    # lines/blocks in visual order even when the HTML nesting is messy.
    lines = [l.strip() for l in soup.get_text("\n").split("\n") if l.strip()]
    all_known_labels = {lbl.lower() for group in SPEC_LABELS.values() for lbl in group}
    for label in labels:
        for i, line in enumerate(lines):
            if line.lower() == label.lower() and i + 1 < len(lines):
                nxt = lines[i + 1]
                if nxt.lower() not in all_known_labels and len(nxt) < 120:
                    return nxt
    return ""


def find_city_hwy_mpg(soup) -> str:
    """The real site shows city/highway MPG as two separate small
    stats (e.g. 'CITY' / '19' and 'HWY' / '25') rather than one
    combined 'Fuel Economy' value — grabbing them together as generic
    label/value pairs was producing garbled results like 'CITY27'."""
    lines = [l.strip() for l in soup.get_text("\n").split("\n") if l.strip()]
    city, hwy = "", ""
    for i, line in enumerate(lines):
        if re.match(r"^city$", line, re.I) and i + 1 < len(lines):
            m = re.match(r"^\d+$", lines[i + 1])
            if m:
                city = lines[i + 1]
        if re.match(r"^(hwy|highway)$", line, re.I) and i + 1 < len(lines):
            m = re.match(r"^\d+$", lines[i + 1])
            if m:
                hwy = lines[i + 1]
    if city and hwy:
        return f"{city} city / {hwy} hwy"
    return ""
...
def find_video_url(soup) -> str:
    """The site embeds each vehicle's YouTube video as a data-videourl
    attribute (confirmed directly from the page's raw HTML) — e.g.
    data-videourl="https://www.youtube.com/watch?v=XXXXXXXXXXX"."""
    for tag in soup.find_all(attrs={"data-videourl": True}):
        url = tag["data-videourl"]
        if "youtube.com" in url or "youtu.be" in url:
            return url
    return ""


def youtube_embed_url(watch_url: str) -> str:
    m = re.search(r"[?&]v=([\w-]+)", watch_url) or re.search(r"youtu\.be/([\w-]+)", watch_url)
    return f"https://www.youtube.com/embed/{m.group(1)}" if m else ""


def find_link_containing(soup, needle: str) -> str:
    """Finds the first <a href> containing the given text — used to grab
    the real financing-application and AutoCheck report links straight
    from each vehicle's own page, rather than guessing/reconstructing
    them (both include tokens that look session- or account-specific,
    so reusing the site's own real link is the reliable approach)."""
    a = soup.find("a", href=re.compile(re.escape(needle), re.I))
    return a["href"] if a else ""


def find_gallery_photos(soup, page_url: str, fallback_photo: str) -> list:
    """Collects vehicle photos shown on the detail page. Restricted to
    the same image host as the vehicle's own listing thumbnail — the
    dealership's logo, the AutoCheck badge, and generic icon graphics
    live on different hosts/paths, and an earlier looser 'any cdn
    image' filter was pulling those in by mistake.

    Many photo carousels only put the FIRST image in a plain <img src>
    and lazy-load the rest via JavaScript (data-src attributes, or a
    list of URLs embedded in a <script> tag) — a plain page fetch can't
    run that JavaScript, so this also checks those common lazy-load
    spots to catch the full gallery instead of just photo #1."""
    photos = []
    seen = set()

    expected_host = ""
    if fallback_photo:
        m = re.match(r"^(https?://[^/]+)", fallback_photo)
        if m:
            expected_host = m.group(1)

    def is_real_photo(src: str) -> bool:
        if not src or not re.search(r"\.(jpg|jpeg|png|webp)", src, re.I):
            return False
        if re.search(r"logo|icon|badge|autocheck", src, re.I):
            return False
        if expected_host and not to_absolute(src, page_url).startswith(expected_host):
            return False
        if not expected_host and "cdn" not in src:
            return False
        return True

    def add(src: str):
        abs_src = to_absolute(src, page_url)
        key = re.sub(r"/\d{2,4}x\d{2,4}/", "/", abs_src)
        if key not in seen:
            seen.add(key)
            photos.append(upsize_image(abs_src))

    # 1) Check every attribute on every tag for something that looks
    # like a real photo URL. Broadened deliberately: the site's lazy-
    # load library (confirmed via a browser Network-tab check — these
    # loaded from disk cache, with "lazy-load.js" as the initiator,
    # meaning the URLs were already in the page's HTML) may use an
    # attribute name we haven't guessed correctly (data-src worked for
    # some sites, but evidently not this one) — scanning everything is
    # slower but far more reliable than guessing one attribute at a time.
        ...
        for attr_val in tag.attrs.values():
            ...
            for val in values:
                if not isinstance(val, str):
                    continue
                # srcset-style values can contain multiple "url size," entries
                for piece in val.split(","):
                    candidate = piece.strip().split(" ")[0]
                    if is_real_photo(candidate):
                        add(candidate)

    # 2) URLs embedded in <script> tags — common for JS-driven carousels
    # that lazy-load the rest of the gallery after page load.
    ...
    # 3) Thumbnails are often wrapped in a link pointing to the
    # full-size photo (a common lightbox/gallery pattern) — check
    # <a href> targets too, not just <img> tags.
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if is_real_photo(href):
            add(href)

    # 4) Reconstruct from bare photo-ID hashes. Confirmed via a live
    # browser Network-tab check: every photo URL follows the pattern
    # {host}/{32-char hex hash}/{size}/{filename}.jpg — only that hash
    # segment changes between photos of the same vehicle. It's likely
    # the page's JavaScript stores just the bare hashes somewhere (a
    # script tag) and builds the full URL at runtime, which is why
    # searching for complete quoted URLs earlier found nothing. This
    # searches for that specific hash pattern instead and reconstructs
    # the full URL using the one photo URL we already know is real.
    if fallback_photo:
        m = re.match(r"^(https?://[^/]+)/([0-9a-f]{16,40})/(\d+x\d+)/([^/?#]+\.(?:jpg|jpeg|png|webp))", fallback_photo, re.I)
        if m:
            host, known_hash, size_folder, filename = m.groups()
            hash_len = len(known_hash)
            found_hashes = set()
            for script in soup.find_all("script"):
                text = script.string or ""
                if not text:
                    continue
                for hm in re.finditer(r"\b[0-9a-f]{" + str(hash_len) + r"}\b", text, re.I):
                    found_hashes.add(hm.group(0))
            for h in found_hashes:
                candidate = f"{host}/{h}/{size_folder}/{filename}"
                add(candidate)
...
def extract_detail_fields(vehicle: dict) -> dict:
    ...
    enriched["video_url"] = find_video_url(soup)
    enriched["finance_url"] = find_link_containing(soup, "ssfinance.aspx") or find_link_containing(soup, "secure.carsforsale.com")
    enriched["autocheck_url"] = find_link_containing(soup, "autocheck.com")

    for field, labels in SPEC_LABELS.items():
        found = find_label_value(soup, labels)
        if field == "body_style":
            # Don't overwrite the reliable, category-derived body style
            # (set in pass 1) with an empty/unreliable scrape result.
            if found:
                enriched[field] = found
        else:
            enriched[field] = found

    better_mpg = find_city_hwy_mpg(soup)
    if better_mpg:
        enriched["mpg"] = better_mpg

    # Mileage: try the same reliable label-based lookup used for the
    # other specs first (safe, since "Mileage" is a distinct label —
    # unlike the earlier whole-page price/mileage scan, which wasn't
    # safe because it also picked up the unrelated "$200 documentary
    # fee" text). Only fall back to the category card's mileage if the
    # detail page truly doesn't have it.
    better_mileage = find_label_value(soup, ["Mileage", "Miles"])
    if not better_mileage:
        page_text = soup.get_text(" ", strip=True)
        better_mileage = extract_mileage(page_text)
    if better_mileage:
        enriched["mileage"] = re.sub(r"[^\d]", "", better_mileage)
...
def enrich_all_vehicles(vehicles: list) -> list:
    ...
    for v in vehicles:
        try:
            ...
            fallback.setdefault("video_url", "")
            fallback.setdefault("finance_url", "")
            fallback.setdefault("autocheck_url", "")
...
def format_mileage(raw: str) -> str:
...
def render_vehicle_card(v: dict) -> str:
    ...
    price_raw = int(v["price"] or 0)
    price_display = f"{price_raw:,}"
    body = v.get("body_style") or ""
    return f'''
      <article class="tag" data-make="{html.escape(make)}" data-price="{price_raw}" data-body="{html.escape(body)}">
        <div class="string"></div>
        <div class="photo"><img src="{html.escape(v['photo'])}" alt="{html.escape(v['title'])}"></div>
        <div class="body">
          <h3>{html.escape(v['title'])}</h3>
          <div class="specs"><span>{format_mileage(v['mileage'])} mi</span></div>
          <div class="price-row">
...
<style>
  ...
  .vd-share{{display:flex;gap:10px;margin-top:16px;}}
  .vd-share a{{flex:1;text-align:center;padding:10px;border-radius:var(--radius-sm);border:1px solid var(--line);font-size:13px;font-family:var(--display);text-transform:uppercase;letter-spacing:0.04em;color:var(--ink);}}
  .vd-share a:hover{{background:var(--concrete);}}
</style>
...
{desc_section}{video_section}        <div class="vd-features">
...
{features_html}
      ...
      <aside class="vd-sidebar">
        ...
        <a class="btn btn-outline" href="contact.html?vehicle={vehicle_query}">Ask a Question</a>
        <div class="divider"></div>
{finance_html}{autocheck_html}        <div class="vd-share">
          <a href="#" onclick="window.open('https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent(window.location.href),'_blank');return false;">Facebook</a>
          <a href="#" onclick="window.open('https://twitter.com/intent/tweet?url='+encodeURIComponent(window.location.href)+'&text='+encodeURIComponent(document.title),'_blank');return false;">Twitter</a>
...
def render_detail_page(v: dict) -> str:
    ...
    spec_rows = [
        ("Mileage", format_mileage(v.get("mileage", "")) + " mi"),
...
'''
    ...
    embed_url = youtube_embed_url(video_url) if video_url else ""
    if embed_url:
        video_section = f'''        <div class="vd-desc">
          ...
          <div style="position:relative;padding-bottom:56.25%;height:0;border-radius:var(--radius-sm);overflow:hidden;">
            <iframe src="{html.escape(embed_url)}" title="Video walkaround of {html.escape(v['title'])}" style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;" allowfullscreen></iframe>
          </div>
        </div>
...
'''
    ...
    finance_url = v.get("finance_url", "")
    if finance_url:
        finance_html = f'        <a class="btn btn-outline" href="{html.escape(finance_url)}" target="_blank" rel="noopener">Apply for Financing</a>\n'
    else:
        finance_html = '        <p style="font-size:13px;">Interested in financing? <a href="finance.html" style="color:var(--amber);text-decoration:underline;">Get pre-qualified</a> before you visit.</p>\n'

    autocheck_url = v.get("autocheck_url", "")
    if autocheck_url:
        autocheck_html = f'        <a class="btn btn-outline" href="{html.escape(autocheck_url)}" target="_blank" rel="noopener">View AutoCheck Report</a>\n'
    else:
        autocheck_html = ""

    vehicle_query = urllib.parse.quote(v["title"])

    schema = {
        "@context": "http://schema.org",
        "@type": "Vehicle",
        "name": v["title"],
        "image": photos[:5] if photos else ([main_photo] if main_photo else []),
        "vehicleIdentificationNumber": v.get("vin") or None,
        "mileageFromOdometer": {
            "@type": "QuantitativeValue",
            "value": v.get("mileage") or None,
            "unitCode": "SMI",
        } if v.get("mileage") else None,
        "offers": {
            "@type": "Offer",
            "price": price.replace(",", ""),
            "priceCurrency": "USD",
            ...
            "seller": {
                "@type": "AutoDealer",
                "name": "IN-n-OUT Auto Sales",
                "telephone": DEALER_PHONE_DISPLAY,
                "address": "7602 River Road East, Puyallup, WA 98371",
            },
        },
    }
    # Drop empty/None fields rather than emitting nulls into the schema.
    schema = {k: v2 for k, v2 in schema.items() if v2 not in (None, [], "")}
    ...
    return DETAIL_PAGE_TEMPLATE.format(
        ...
        autocheck_html=autocheck_html,
        schema_json=schema_json,
        vehicle_query=vehicle_query,
        mileage=format_mileage(v.get("mileage", "")),
    )
