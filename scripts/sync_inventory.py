...
SPEC_LABELS = {
    ...
    "exterior_color": ["Exterior Color", "Ext. Color", "Color"],
...
def find_section_list_items(soup, heading_keywords: list) -> list:
    ...
    and returns the <li> items in the list that follows it."""
    heading = soup.find(
        ["h1", "h2", "h3", "h4", "strong", "b"],
        string=re.compile("|".join(heading_keywords), re.I),
    )
    ...
    for sib in heading.find_all_next():
        ...
        if sib.name in ("ul", "ol"):
            return [li.get_text(strip=True) for li in sib.find_all("li") if li.get_text(strip=True)]
    return []
...
def extract_detail_fields(vehicle: dict) -> dict:
    ...
    # A detail page's own price/mileage text is the most authoritative
    # version — prefer it over the summary card's if found.
    page_text = soup.get_text(" ", strip=True)
    better_price = extract_price(page_text)
    if better_price:
        enriched["price"] = better_price
    better_mileage = extract_mileage(page_text)
    if better_mileage:
        enriched["mileage"] = better_mileage
...
def render_detail_page(v: dict) -> str:
    ...
    spec_rows = [
        ...
        ("Body Style", v.get("body_style") or "—"),
    ]
    spec_cards = "\n".join(
    ...
    description = v.get("description") or (
        "Contact us for the full description and condition details on this vehicle."
    )
...
def write_detail_pages(vehicles: list) -> None:
    for v in vehicles:
