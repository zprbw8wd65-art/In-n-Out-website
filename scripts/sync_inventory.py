...
def find_gallery_photos(soup, page_url: str, fallback_photo: str) -> list:
    ...
    for img in soup.find_all("img"):
        ...
        if expected_host:
            if not abs_src.startswith(expected_host):
                continue
        elif "cdn" not in src:
            continue

        # Skip anything that isn't a real photo path (logos/icons on the
        # same CDN sometimes live under short generic filenames).
